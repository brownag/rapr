#' Query RAP Yearly Production Data
#'
#' Retrieves remotely sensed production estimates from the Rangeland Analysis
#' Platform (RAP) using the `productionV3` API endpoint. This function
...
#'
#' # all years (year=NULL)
#' res <- get_rap_production_table(aoi)
#' str(res)
#'
#' # specific year
#' res <- get_rap_production_table(aoi, year = 1992)
#' str(res)
#'
#' # multiple specific years
#' res <- get_rap_production_table(aoi, year = 1993:2003)
#' str(res)
#'
#' # 1 kilometer buffer around point
#' res <- get_rap_production_table(terra::buffer(aoi, 1000), year = 2004)
#' str(res)
#'
get_rap_production_table <- function(aoi, year = NULL, mask = TRUE, nodata_flag = NA_real_) {
  ...
  res <- lapply(seq_len(nrow(grd)), function(i) {
    ...
    rap_json <- httr::RETRY(verb = "POST",
                            url = "https://us-central1-rap-data-365417.cloudfunctions.net/productionV3",
    ...
    if (!is.null(content)) {
      prod <- content$properties$production
